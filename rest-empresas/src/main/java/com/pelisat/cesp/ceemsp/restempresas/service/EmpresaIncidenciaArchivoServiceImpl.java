...
@Service
public class EmpresaIncidenciaArchivoServiceImpl implements EmpresaIncidenciaArchivoService {
    ...
    Logger logger = LoggerFactory.getLogger(EmpresaIncidenciaArchivoService.class);

    @Autowired
    public EmpresaIncidenciaArchivoServiceImpl(
            IncidenciaArchivoRepository incidenciaArchivoRepository, UsuarioService usuarioService,
    ...
    @Override
    public File descargarArchivoIncidencia(String incidenciaUUid, String archivoUuid) {
        if(StringUtils.isBlank(incidenciaUUid) || StringUtils.isBlank(archivoUuid)) {
    ...
    @Override
    @Transactional
    public IncidenciaArchivoMetadata agregarArchivoIncidencia(String incidenciaUuid, String username, MultipartFile multipartFile) {
        if(StringUtils.isBlank(incidenciaUuid) || StringUtils.isBlank(username) || multipartFile == null) {
    ...
    @Override
    @Transactional
    public IncidenciaArchivoMetadata eliminarArchivoIncidencia(String incidenciaUuid, String archivoIncidenciaUuid, String username) {
        if(StringUtils.isBlank(incidenciaUuid) || StringUtils.isBlank(archivoIncidenciaUuid) || StringUtils.isBlank(username)) {
            logger.warn("Alguno de los parametros viene como nulo o vacio");
            throw new InvalidDataException();
        }
